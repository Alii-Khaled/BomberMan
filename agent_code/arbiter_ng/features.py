"""ARBITER-NG (E101) input: flat vector = 12x17x17 board tensor (raveled,
the lossless apex channels) ++ the 98-dim scalar feature vector. This is
the interface the callbacks/search already expect: `state_to_features`
returns one flat float32 vector; the NG model slices it internally.
Dihedral augmentation is applied by the training script (spatial numpy
transforms on the tensor + AUG_PERMS on the scalars + map_action).
"""
...
SCALAR_DIM = 98
FEATURE_DIM = 12 * 17 * 17 + SCALAR_DIM
...
def _build_aug_perms():
    ...
    perms = np.tile(np.arange(SCALAR_DIM, dtype=np.int64), (N_SYMS, 1))
    ...
    assert all(len(set(row)) == SCALAR_DIM for row in perms), "perm not bijective"
    return perms
...
def scalar_features(game_state, safety_info=None, own_bomb=None):
    ...
    f = np.zeros(SCALAR_DIM, dtype=np.float32)
...
def board_tensor(game_state, safety_info=None, power=3):
    ...
    if safety_info is not None and 'danger' in safety_info:
        ...
        for k in range(3):
            if d.shape[0] > k:
                t[8 + k] = d[k].astype(np.float32)
        for (cx, cy) in (safety_info.get('blast_if_bomb') or ()):
...
def state_to_features(game_state, safety_info=None, own_bomb=None):
    ...
    s = scalar_features(game_state, safety_info, own_bomb)
    return np.concatenate([t, s]).astype(np.float32)


def transform_tensor(t, sym):
    """Dihedral spatial transform of a (C,17,17) tensor (matches
    transform_state's coordinate map exactly)."""
    A, b = SYMS[sym]
    xs, ys = np.meshgrid(np.arange(17), np.arange(17), indexing='ij')
    nx = A[0, 0] * xs + A[0, 1] * ys + b[0]
    ny = A[1, 0] * xs + A[1, 1] * ys + b[1]
    out = np.zeros_like(t)
    out[:, nx, ny] = t[:, xs, ys]
    return out
