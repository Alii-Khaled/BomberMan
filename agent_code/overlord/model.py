...
def build_model(in_ch=N_CHANNELS, base=None, n_actions=N_ACTIONS,
    ...
    if fc_dim is None:
        fc_dim = _env_int('OVERLORD_FC', 512, 128, 2048)
    if norm is None:
        norm = _env_str('OVERLORD_NORM', 'bn')
        if norm not in ('bn', 'gn'):
            norm = 'bn'
    if deep is None:
        deep = os.environ.get('OVERLORD_DEEP', '0') == '1'
    return OverlordNet(in_ch=in_ch, base=base, n_actions=n_actions,
